...
(dotest
  (with-debug-tag :dbg-01
   ;(with-system-err-str) ; capture jetty logging from System/err
    (let [ctx-out (demo.hello/invoke-interceptors {} [respond-hello-intc])]
...
(dotest
  (with-debug-tag :dbg-02
   ;(with-system-err-str) ; capture jetty logging from System/err
    (demo.hello/with-service tst-service-map ; mock testing w/o actually starting jetty
      (let [resp (demo.hello/service-get "/greet")]
...
(dotest
  (with-debug-tag :dbg-03
    (let [sys-err-str (with-system-err-str ; capture jetty logging from System/err
                        (demo.hello/with-server tst-service-map ; test over http using jetty server
  ...
  (with-debug-tag :dbg-04
    (with-system-err-str ; capture jetty logging from System/err
      (demo.hello/with-service tst-service-map ; mock testing w/o actually starting jetty
        ...
        (let [resp (demo.hello/service-get "/greet?name=Michael")]
        ...
        (with-debug-tag :dbg-04-a
          (let [resp (demo.hello/service-get "/greet?name=")]
            (destruct [resp {:status ? :body ?}]
              (spyx status)
              (is= status 200)
        ...
        (let [resp (demo.hello/service-get "/greet?name=YHWH")]
          (destruct [resp {:status ? :body ?}]
            (is= status 404)
            (is-nonblank= body "Not found")))

        (let [resp (demo.hello/service-get "/greet?name=Voldemort")]
          (destruct [resp {:status ? :body ?}]
            (is= status 404)
            (is-nonblank= body "Not found")))
...
(dotest
  (with-debug-tag :dbg-05
    ...
    (demo.hello/with-service tst-service-map ; mock testing w/o actually starting jetty
      ...
      (let [resp (demo.hello/service-get "/greet" :headers {tp/accept tp/application-edn})]
        (is= (grab :status resp) 200)
